...
//
//def id2json(id: EntityId[_, _]): JsValue = id match
...
//    
//def primitive2json(field: RootPrimitiveValue[_, _]): JsValue = field match
...
//    )
//    case ref: BackReferenceValue[_] => JsObject(
...
//
//def entityd2json(entity: Entity[_, _, _]): JsObject = JsObject(
...
//        
//
//def json2field(json: JsValue): RootPrimitiveValue[_, _] = json match
...
//
//
//implicit object EntityIdFormat extends RootJsonFormat[EntityId[_, _]]:
//    def write(id: EntityId[_, _]): JsValue = id2json(id)
//    def read(value: JsValue): EntityId[_, _] = value match
//        case JsNumber(value) => LongId(value.toLong)
//        case JsString(str) => StringId(str)
//        case _ => deserializationError("EntityId expected")
//    
//
//implicit object EntityFormat extends RootJsonFormat[Entity[_, _, _]]:
//    def write(entity: Entity[_, _, _]): JsObject = entityd2json(entity)
//    def read(json: JsValue): Entity[_, _, _] = json.asJsObject.getFields("id", "fields") match
//        case Seq(id, fields) => ???
//        case _ => deserializationError("Entity expected")
//    
//
//implicit val entityListFormat: RootJsonFormat[Seq[Entity[_, _, _]]] = new RootJsonFormat[Seq[Entity[_, _, _]]]:
//    def write(entities: Seq[Entity[_, _, _]]): JsValue = JsArray(entities.map(entityd2json).toVector)
//    def read(json: JsValue): Seq[Entity[_, _, _]] = json match
//        case JsArray(array) => array.map(_.convertTo[Entity[_, _, _]]).toList
