...
import my.valerii_timakov.sgql.entity.{AbstractTypeError, SearchConditionParseError, TypeNotFountError}
import my.valerii_timakov.sgql.services.{CrudRepository, FieldsDescriptorsParser, SearchConditionsParser, TypesDefinitionProvider}
...
class CrudActor(
    ...
    : Either[entity.Error, Try[Res]] =
        val parsedDescriptorResult = fieldsDescriptorsParser.parse(getFields)
        flatMap(
